(() => {
  ...
  function recordEvent(name) {
    const event = { name, timestamp: new Date().toISOString(), url: window.location.href, referrer: document.referrer || null, utm: parseUtm() };
  ...
  function clear() {
    try { window.localStorage.removeItem(STORAGE_KEY); renderEventLog('Đã xoá event log cục bộ.'); return true; } catch { renderEventLog('Không thể xoá localStorage trong trình duyệt này.'); return false; }
  }
  ...
  function renderCampaignDetails() {
    ...
    if (!node) return;
    const utm = parseUtm(); node.replaceChildren();
    Object.entries(utm).forEach(([key, value]) => { const row = document.createElement('div'); const label = document.createElement('dt'); const detail = document.createElement('dd'); label.textContent = key; detail.textContent = value || 'không có'; row.append(label, detail); node.append(row); });
  }

  function initialise() {
    document.querySelectorAll('[data-track]').forEach((element) => element.addEventListener('click', () => recordEvent(element.dataset.track)));
